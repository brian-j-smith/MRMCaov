...
! Global variables within the module
 INTEGER, PARAMETER :: CAT_out = 20 ! output file containing categorization data, it is a general parameter 
 CHARACTER(*), PARAMETER:: CAT_out_FILE = "categorization.out" ! within the module categorization, but not outside
...
! create categorical data from the continuoud test results values
 ...
 if(idebug == 1) OPEN(CAT_out, status = 'unknown', access = 'sequential', file = CAT_out_FILE)

 NUM_CASES = NUM_NORMAL_CASES + NUM_ABNORMAL_CASES

if(idebug == 1) then
   WRITE(CAT_out,*) "Total Number of cases:                   ", NUM_CASES
   WRITE(CAT_out,*) "Total Number of actually abnormal cases: ", NUM_ABNORMAL_CASES
   WRITE(CAT_out,*) "Total Number of actually normal cases:   ", NUM_NORMAL_CASES
endif
...
! second dimension is to allow for values and index
 ...
 IF (POSITIVEISLARGE==0) THEN
   IF(idebug == 1) WRITE(CAT_out,*) "Higher probability of abnormality is for smaller values of TEST RESULT VALUEs"
 ...
 ELSE
   IF(idebug == 1) WRITE(CAT_out,*) "Higher probability of abnormality is for larger values of TEST RESULT VALUEs"
   ALL_CASES(1,1:NUM_NORMAL_CASES)              =  NEG_INPUT(1:NUM_NORMAL_CASES)
   ALL_CASES(1,NUM_NORMAL_CASES+1:NUM_CASES)    =  POS_INPUT(1:NUM_ABNORMAL_CASES)
 ENDIF
 ! Load the truth
...
! Write out the number of truth runs found and their properties
if(idebug == 1) then
   WRITE(CAT_out,*) "TOTAL NUMBER OF TRUTH RUNS IN DATA: ", NUM_TRUTH_RUNS
   WRITE(CAT_out,*) "LIST OF TRUTH RUNS FOUND"
   WRITE(CAT_out,*) "  TRUTH    CUTOFF          # ACT.   # ACT.          FALSE POSITIVE  TRUE POSITIVE "
   WRITE(CAT_out,*) "  RUN      VALUE           NORMAL   ABNORMAL        FRACTION (FPF)  FRACTION (TPF) "
   DO  I = 1,NUM_TRUTH_RUNS
       WRITE(CAT_out,'(I4,8x,E14.6,2x,2 (I6,6x),2(F14.6,2x))') I, TRUTH_RUNS_CATEGORIES(TST_RES_VAL,I), &
         INT(TRUTH_RUNS_CATEGORIES(act_neg,I)) , INT(TRUTH_RUNS_CATEGORIES(act_pos,I)), FPF_TPF(1,I), FPF_TPF(2,I)
   ENDDO
   WRITE(CAT_out,*) "END OF LIST OF TRUTH RUNS FOUND"
endif
...
IF (NUM_TRUTH_RUNS > MAX_NUM_CATEGORIES) THEN
     ...
     if(idebug == 1) WRITE(CAT_out,*) "LABROC5 ALGORITHM WILL BE USED AIMING FOR A TOTAL OF ", &
                MAX_NUM_CATEGORIES," CATEGORIES "
...
ELSE ! In this case the number of categories is the same as the number of truth runs 
     ...
     if(idebug == 1) WRITE(CAT_out,"('LABROC5 ALGORITHM WILL BE NOT BE USED, ALL THE ',I3, &
                           &' TRUTH RUNS WILL BE USED')") NUM_TRUTH_RUNS
...
if(idebug == 1) then
 WRITE(CAT_out,*) "THE TOTAL NUMBER OF CATEGORIES THAT WILL BE USED IS", NUM_CATEGORIES
 WRITE(CAT_out,*) "    out OF A TOTAL OF ", NUM_TRUTH_RUNS, " TRUTH RUNS"
endif
...
! Write out the categories that has been created by  the categorization process
if(idebug == 1) then
   WRITE(CAT_out,*) "LIST OF BOUNDARIES THAT WILL BE USED IN THE MLE CALCULATION"
   WRITE(CAT_out,*) "    CATEGORY  cutoff value    ACC. NORMAL CASES      ACC. ABNORMAL CASES"
   DO I = 1, NUM_CATEGORIES - 1
      WRITE(CAT_out,*) I, TST_RES_VAL_CATEGORY_BOUNDARY(I) , CAT0(ACT_NEG,I), CAT0(ACT_POS,I)
   ENDDO
   WRITE(CAT_out,*) I, "  INFINITY             " ,  CAT0(ACT_NEG,I), CAT0(ACT_POS,I)
endif
...
! will be removed and I will be able to live with a scalar.
  ...
  IF( CITY_BLOCK_DIST(2,POINT_CLOSEST_CENTER) .speq. -1.0_double) THEN  
    ...
    if(idebug == 1) WRITE(CAT_out,*)"SELECTED POINT closest to diagonal ", &
                                    POINT_CLOSEST_CENTER, D_R_PRIME(POINT_CLOSEST_CENTER)
  ELSE
    if(idebug == 1) WRITE(CAT_out,*)"POINT closest to diagonal WAS ALREADY SELECTED ", &
                                    POINT_CLOSEST_CENTER, D_R_PRIME(POINT_CLOSEST_CENTER)
  ENDIF
...
! WRITE out THE DATA ABout THE POINTS THAT HAVE BEEN SELECETED THROUGH THE "COVERAGE" PART OF THE ALGORITHM.
if(idebug == 1) then
   WRITE(CAT_out,*)
   WRITE(CAT_out,"('TRUTH RUNS AFTER UNIFORMLY SPACED GRID SELECTION :',I3)") SELECTED_BOUNDARIES
   WRITE(CAT_out,*) "TRUTH     CITY_BLOCK    STRETCHED CITY      SELECTION "
   WRITE(CAT_out,*) "RUN #     DIST.         BLOCK DIST.         STATUS "
   WRITE(CAT_out,*)
   DO I= 1 , NUM_TRUTH_RUNS
     IF(CITY_BLOCK_DIST(2,I)>0.0_double) &
                    WRITE(CAT_out,'(I4,3x,2(f12.6,2x),10x,"SELECTED")') I, CITY_BLOCK_DIST(1,I), D_R_PRIME(I) 
     IF(CITY_BLOCK_DIST(2,I)<0.0_double) &
                   WRITE(CAT_out,'(I4,3x,2(f12.6,2x),10x,"NOT TAKEN")') I, CITY_BLOCK_DIST(1,I), D_R_PRIME(I) 
   ENDDO
endif
...
!  POINTS ARE SELECTED.
 ...
 IF(SELECTED_BOUNDARIES < NUM_CATEGORIES - 1) THEN
    ...
    if(idebug == 1) then
      WRITE(CAT_out,*)
      WRITE(CAT_out,*) "TRUTH RUNS ADDED TO THE UNIFORMLY SPACED GRID SELECTION: ", POINTS_TO_BE_ADDED
      WRITE(CAT_out,*) "TRUTH     CITY_BLOCK    STRETCHED CITY      SELECTION "
      WRITE(CAT_out,*) "RUN #     DIST.         BLOCK DIST.         STATUS "
    endif
...
! Start the category counter, which is used to update the boundary/threshold value and the category array  
 ...
 ENDDO
  ...
  ! this is a check to see if the input arrays were meaningfully constructed.
  ! Remove comments if you are changing the categorizer
  !IF(I_CATEGORY /= 1) THEN
  !  WRITE(CAT_out,*) "ERROR:: CREATE_ORDINAL_CATEGORIES"
  !  WRITE(CAT_out,*) NUM_CATEGORIES - I_CATEGORY, NUM_CATEGORIES
  !ENDIF
...
! SELECTED POINT
 ...
 ! LOOK INSIDE OF THE LARGEST GAP, AND FIND THE POINT WHICH IS MOST DISTANT FROM ITS CLOSEST EXTREME
  ...
  if(idebug == 1) WRITE(CAT_out,'(I4,3x,2(f12.6,2x),10x,"SELECTED")') THE_POINT, CITY_BLOCK_DIST(1,THE_POINT), & 
                       D_R_PRIME(THE_POINT) 
