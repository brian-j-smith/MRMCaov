...
The study employed a factorial design in which each of the five radiologists read and rated both the CINE and SE MRI images from all 114 cases.  The original study variables in the `VanDyke` data frame are summarized in Table~\ref{tbl:VanDyke} along with two additional `case2` and `case3` variables that represent hypothetical study designs in which cases are nested within readers (`reader`) and within imaging modalities (`treatment`), respectively.
